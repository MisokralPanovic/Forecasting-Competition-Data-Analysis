...
aggregated_dt |> head(20) |> paged_table()
```
...
barrier_dt |> head(20) |> paged_table()
```
\
...
# append question metadata (`answer resolved probability` column)
aggregated_day1_dt  |> head(20) |> paged_table()
```
...
final_table_new_method[, .N, by = Best_Method][order(-N)][, Percentage := (N / sum(N)) * 100] |> head(20) |> paged_table()
```
\
